...
from bids_phase_unwrap import bids_phase_unwrap, bids_phase_unwrap_docker, find_subjects_and_sessions
import time
...
class MainWindow(QMainWindow):
    ...
    def __init__(self, parent, add_info):
        ...
        self.setWindowTitle("Unwrap Phase image")
        ...
        self.pipeline = "phase_unwrap"
        
        sss_slurm = self.add_info.get('sss_slurm')
        
        if sss_slurm == None:
            print('no sss slurm')
            self.tab = PhaseUnwrapTab(self, sss_slurm)
            layout = QVBoxLayout()
            ...
            self.main_tab = PhaseUnwrapTab(self, sss_slurm)
...
class PhaseUnwrapTab(QWidget):
    ...
    def __init__(self, parent, sss_slurm=None):
        ...
        self.select_phase_button = QPushButton("Select wrapped phase image")
        self.select_phase_button.clicked.connect(self.select_phase)
        self.phase_label = QLabel()
        
        self.deriv = QLineEdit(self)
        self.deriv.setPlaceholderText("output derivative folder (default: phase_unwrap)")
        
        self.out_name = QLineEdit(self)
        self.out_name.setPlaceholderText("output img name (default: add rec-unwrapped)")
        ...
        self.button = QPushButton("Unwrap Phase")
        self.button.clicked.connect(self.action)
        
        layout = QGridLayout()
        layout.addWidget(self.select_phase_button, 0, 0, 1, 1)
        layout.addWidget(self.phase_label, 0, 1, 1, 1)
        layout.addWidget(self.deriv, 1, 0, 1, 2)
        layout.addWidget(self.out_name, 2, 0, 1, 2)
        layout.addWidget(self.subjects_input, 3, 0, 1, 2)
        layout.addWidget(self.sessions_input, 4, 0, 1, 2)
    ...
    def select_phase(self):
        ...
        """
        self.phase_path, _ = QFileDialog.getOpenFileName(self, "Select wrapped phase image", self.bids.root_dir, options=QFileDialog.DontUseNativeDialog)
        if self.phase_path == '':
            return
        
        filename = os.path.basename(self.phase_path)
        ...
        self.phase_label.setText(seq_name)
    ...
    def action(self):
        ...
        args = []
        args.extend(['--phase-path', self.phase_path])
        if deriv:
        ...
        if self.local:
            ...
            self.thread = QThread()
            self.action = ActionWorker(self.bids.root_dir, sub, ses, self.pipeline, self.phase_path, deriv=deriv, out_name=out_name, use_docker=use_docker)
            ...
            self.is_in_progress(('phase_unwrap', True))
            jobs_submitted = []
            ...
            self.is_in_progress(('phase_unwrap', False))
...
class ActionWorker(QObject):
    ...
    def __init__(self, bids, sub, ses, pipeline, phase_path, deriv=None, out_name=None, use_docker=False):
        """
    ...
    def run(self):
        ...
        for sub, sess in subjects_and_sessions:
            for ses in sess:
                ...
                if self.use_docker:
                    print('use_docker')
                    bids_phase_unwrap_docker(self.bids, sub, ses, self.phase_path, **kwargs)
                else:
                    bids_phase_unwrap(self.bids, sub, ses, self.phase_path, **kwargs)
